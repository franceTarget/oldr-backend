...
public class ValueUtil {
    ...
    private static final String MOBILE =
            "(?i)Mobile|iP(hone|od|ad)|okhttp|Android|BlackBerry|Blazer|PSP|UCWEB|IEMobile|Kindle|NetFront|Silk-Accelerated|(hpw|web)OS|Fennec|Minimo|Opera M(obi|ini)|Dol(f|ph)in|Skyfire|Zune";
    /**
     * 是否是 iOS 端
     */
     ...
     */
    public static boolean greater0(Number num) {
     ...
     */
    public static boolean less0(Number num) {
        return !greater0(num);
    }
    ...
    private static final String[] UNIT = new String[]{"B", "KB", "MB", "GB", "TB", "PB", "EB", "ZB", "YB", "BB"};

    /**
     * 字节长度转换成人类友好的可读形式
     */
    public static String humanRead(final Number byteLength) {
        Number tmp = byteLength;
        for (String unit : UNIT) {
            if (tmp.doubleValue() < 1024) {
                return String.format("%.2f %s", tmp.doubleValue(), unit);
            }
            tmp = tmp.doubleValue() / 1024;
        }
        return byteLength + SPACE + UNIT[0];
    }
    // ========== number ==========
    ...
    public static String getSuffix(String image) {
        return (isNotBlank(image) && image.contains(".")) ? image.substring(image.lastIndexOf(".")) : EMPTY;
    }
    ...
    public static String getPrefix(String image) {
        return (isNotBlank(image) && image.contains(".")) ? image.substring(0, image.lastIndexOf(".")) : EMPTY;
    }

    public static String like(String param) {
        return isBlank(param) ? ValueUtil.EMPTY : LIKE + param + LIKE;
    }

    public static String leftLike(String param) {
        return isBlank(param) ? ValueUtil.EMPTY : LIKE + param;
    }

    public static String rightLike(String param) {
        return isBlank(param) ? ValueUtil.EMPTY : param + LIKE;
    }
    // ========== object & string ==========
    ...
    public static String getField(Object data, String field) {
        ...
        if (value == null) {
            ...
            // 如果是日期, 则格式化
            return getNil(DateUtil.formatDateToString((Date) value));
        } else {
            return getNil(value);
        }
    }
    ...
    // }
    // /** 条件为 true 则抛出业务异常 */
    // public static void assertException(boolean flag, ErrorCodeEnum errorCodeEnum){
    // if (flag) throw new ServiceException(errorCodeEnum);
    // }
    //
    // /** 条件为 true 则抛出律师未认证异常 */
    // public static void assertLawyerAuthException(boolean flag) {
    // if (flag) throw new LawyerAuthException();
    // }
    //
    // /** 条件为 true 则抛出没找到异常 */
    // public static void assertNotFoundException(boolean flag, String msg) {
    // if (flag) throw new NotFoundException(msg);
    // }
    //
    // /** 条件为 true 则抛出必须处理的异常 */
    // public static void assertMustHandleException(boolean flag, String msg) throws ServiceMustHandleException {
    // if (flag) throw new ServiceMustHandleException(msg);
    // }
    // ========== assert ==========
